...
| 专项流程优先级 | Full | Debug、TDD、验证、review 优先走 CE/GSD/gstack；Superpowers 只兜底 |
...
直接删掉即可。其他章节不依赖于它。如果你不用 CE、GSD 或 gstack，可以删掉对应路由行，保留独立 Forge 流程。
...
从 Essential 开始。使用一周。当你需要 CE/GSD/gstack 路由、局部护栏、hooks 指南或更强的爆炸半径规则时，再升级到 Full。
