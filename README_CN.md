...
**Router-first 指令模板 _外加_ Crucible —— 让 Agent 从自己的历史失败中学习的本机记忆系统。**

Forge 生成简洁的 `CLAUDE.md` / `AGENTS.md`，把工作流路由到 CE / GSD / gstack / Waza / Revolve。**Crucible** 是 Forge 的演化资产系统，由 `Stop` hook 把每次工具错误按 fingerprint 写进 `failed-directions/` 库，再由 `PreToolUse` hook 在高风险命令执行**前**同步检索：命中则 deny 并把已验证的 `correct_action` 返回给 Agent —— 让破坏性命令被拦在执行之前，而不是事后回滚。

[快速开始][quick-start] •
[Crucible][crucible-section] •
[运行时 Hooks][runtime-hooks-section] •
[路由模型][routing-model] •
[模板][templates] •
[测试][testing]
...
- [模板][templates]
- [运行时 Hooks][runtime-hooks-section]
- [Crucible · 演化资产系统][crucible-section]
...
Forge 还内置 **[Crucible][crucible-section]** —— 一套本机错误记忆系统：把每次 Agent 反复踩的工具失败按 fingerprint 沉淀成 yaml，并通过 `PreToolUse` hook 在高风险命令执行前同步拦截：命中已知失败模式则 deny 并把已验证的 `correct_action` 返回给 Agent。模板 + 运行时 hook + Crucible 三层合在一起：**短指令、强约束、长记忆**。
...
[routing-model]: #路由模型
[templates]: #模板
[runtime-hooks-section]: #运行时-hooks
[crucible-section]: #crucible--演化资产系统
